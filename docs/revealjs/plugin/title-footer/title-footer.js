...
title_footer.getElementsByTagNames = function (list, obj) {
    ...
    resultArray.sort((a, b) => {
        if (a.sourceIndex) return a.sourceIndex - b.sourceIndex;
        if (a.compareDocumentPosition) return 3 - (a.compareDocumentPosition(b) & 6);
        return 0;
    });
    return resultArray;
};
...
title_footer.initialize = function (title, footer, background) {
    ...
    // If the title is empty, try to get it from the first section
    if (this.title === '') {
        var first_section = document.querySelector('section');
        if (first_section.querySelector('section')) {
            first_section = first_section.querySelector('section');
        }
        var title_elements = this.getElementsByTagNames('h1,h2,h3,h4,h5', first_section);
        if (title_elements.length > 0) {
            this.title = title_elements[0].textContent;
            for (var i = 1; i < title_elements.length; i++) {
                this.title += ' - ' + title_elements[i].textContent;
            }
        }
    }

    // Create the Title-Header header
    var createHeaderFooter = function (id) {
        ...
        svg_element.setAttribute('width', '30');
        svg_element.setAttribute('height', '30');
        svg_element.style.marginRight = '10px'; // Add right margin to increase the space between the SVG and the text

        // Append the text and SVG to the paragraph in the correct order
        if (id === 'header') {
            header_footer_p.appendChild(document.createTextNode(this.title));
            header_footer_p.appendChild(svg_element);
        } else {
            header_footer_p.appendChild(svg_element);
            header_footer_p.appendChild(document.createTextNode(this.footer));
        }

        header_footer.appendChild(header_footer_p);

        var div_class_reveal = document.querySelector('.reveal');
        div_class_reveal.appendChild(header_footer);
    }.bind(this);

    createHeaderFooter('header');
    createHeaderFooter('footer');
};
